...
try:
    ...
    # Tunggu konten BNI render
    time.sleep(20)

    # Stop loading sisa resource
...
# =====================================================
# 2. PARSE HTML DAN EKSTRAK DATA
# =====================================================
soup = BeautifulSoup(html, "html.parser")

def extract_table(title_id):
    title = soup.find("span", id=title_id)
    if title is None:
        raise RuntimeError(f"Bagian {title_id} tidak ditemukan.")

    container = title.find_parent("div", class_=lambda x: x and "col-sm-4" in x)
    if container is None:
        raise RuntimeError(f"Container {title_id} tidak ditemukan.")

    table = container.find("table")
    if table is None:
        raise RuntimeError(f"Tabel {title_id} tidak ditemukan.")
    ...
    for row in table_rows:
        ...
        if len(cells) < 3:
            continue
        currency = cells[0].get_text(" ", strip=True).split()[-1]
        buy = cells[1].get_text(strip=True)
        sell = cells[2].get_text(strip=True)
        data[currency] = {"buy": buy, "sell": sell}
    return data

special_rates = extract_table("dnn_ctr6793_BNIValasInfoView_lblTitleCounter")
tt_counter = extract_table("dnn_ctr6793_BNIValasInfoView_lblTitleBankNotes")

print("\nJumlah Special Rates ditemukan:", len(special_rates))
print("Jumlah TT Counter ditemukan   :", len(tt_counter))

if len(tt_counter) != 16:
    raise RuntimeError(f"Data kurs tidak lengkap. Ditemukan {len(tt_counter)} mata uang.")

# =====================================================
# 3. GABUNGKAN DATA DAN BUAT DATAFRAME
# =====================================================
result = []
for currency, counter_rate in tt_counter.items():
    special_rate = special_rates.get(currency, {})
    result.append({
...
df = pd.DataFrame(result, columns=["buyRateCounter", "buyRateERate", "currency",
                                   "sellRateCounter", "sellRateERate"])

rate_cols = ["buyRateCounter", "buyRateERate", "sellRateCounter", "sellRateERate"]
for col in rate_cols:
    df[col] = df[col].astype("string").str.replace(".", "", regex=False).str.replace(",", ".", regex=False)
...
# Convert missing values ke None
